...
FrameLen needs calculated as I(255 - S). For normal frame, S = 0 thus FrameLen = 255*I

                                    31:24           23:16           15:0
    Reed Solomon Status Register    FailCnt[7:0]    FrameCnt[7:0]   ErrCnt[15:0]

                                    31:29       28:24       21:16       13:8        4:0
    Attached Sync Marker (ASM)      BitSlips    IL_BET      OOL_BET     Verifies    FlyWheels   Usually defaulted and ignored by user
            Default
...
`ifndef RS_DEC_CONTROL
    ...
    `define RS_DEC_SOURCE_SELECT   5'b0_11xx
    `define RS_DEC_PHASE_INC       5'b1_00xx
    `define RS_DEC_SYNCWORD        5'b1_01xx
`endif
...
module reedSolomonRegs(
    ...
    output  reg [3:0]   sourceSelect = 4'b0000,
    input       [5:0]   errors,
    ...
    output      [4:0]   IL_BET,
                        ...
                        Verifies,
                        FlyWheels,
    output      [15:0]  Dec_ctrl_tdata,
                        frameLen,
    output  reg [31:0]  phaseInc = 32'h1000_0000,
    output  reg [31:0]  SyncWord = 32'h1acffc1d
    ...
    assign  Dec_ctrl_tdata = {2'b00, control[9:8], 4'b0000, control[7:0]};    // R_IN = 13:8, N_IN = 7:0
    ...
    always @(posedge busClk) begin
        if (cs && wr0) begin
            casex (addr)
                ...
                `RS_DEC_SOURCE_SELECT:   sourceSelect       <= dataIn[3:0];
                `RS_DEC_PHASE_INC:       phaseInc[7:0]      <= dataIn[7:0];
                `RS_DEC_SYNCWORD:        SyncWord[7:0]      <= dataIn[7:0];
                default: ;
            endcase
        end
        ...
        if (cs && wr1) begin
            casex (addr)
                ...
                `RS_DEC_PHASE_INC:       phaseInc[15:8]     <= dataIn[15:8];
                `RS_DEC_SYNCWORD:        SyncWord[15:8]     <= dataIn[15:8];
                default: ;
            endcase
        end
        ...
        if (cs && wr2) begin
            casex (addr)
                ...
                `RS_DEC_PHASE_INC:       phaseInc[23:16]    <= dataIn[23:16];
                `RS_DEC_SYNCWORD:        SyncWord[23:16]    <= dataIn[23:16];
                default: ;
            endcase
        end
        ...
        if (cs && wr3) begin
            casex (addr)
                ...
                `RS_DEC_PHASE_INC:       phaseInc[31:24]    <= dataIn[31:24];
                `RS_DEC_SYNCWORD:        SyncWord[31:24]    <= dataIn[31:24];
                default: ;
            endcase
        end
    end
    ...
    always @* begin
        if (cs) begin
            casex (addr)
                ...
                `RS_DEC_SOURCE_SELECT:  dataOut = {28'b0, sourceSelect};
                `RS_DEC_PHASE_INC:      dataOut = phaseInc;
                `RS_DEC_SYNCWORD:       dataOut = SyncWord;
